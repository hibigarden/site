...
the optional settings page controls the initial insert mode and status visibility. `context.statusBar.register` supplies a mode pill at the editor page's bottom-left, beside the full-height sidebar; command prompts remain in the source pane. view-scoped command contexts, status handles, and preference listeners are cleaned up when the editor or plugin stops. see [the editing guide](../../../docs/guides/vim.md) and [upstream engine](https://github.com/replit/codemirror-vim).

a second pill shows pending normal/visual command keys, then keeps the last completed sequence: `2` → `22` → `22k`. motions execute on their final key; a standalone enter leaves that displayed sequence intact. escape cancels a pending sequence and restores the last one. search and `:` prompts show their current input and retain it on enter. insert-mode document text is not collected. **show vim status** controls both pills; listeners and command state are local to the active source editor and are removed on teardown.
